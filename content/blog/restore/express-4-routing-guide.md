...
Next run this command to install Express as a dependency

```bash
npm install express --save
```

I also like using colors for colorful console logs, so you may want to install colors
...
In the code above we are setting up the router to send a response when the home location is accessed. We then tell our app to use this router for all paths

```javascript
app.use("/", router);
```

Multiple routes can be setup like this
